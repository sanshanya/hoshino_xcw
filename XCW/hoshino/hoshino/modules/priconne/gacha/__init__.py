#使用建议：
#BOT_NICKNAME = r'臭鼬|猫猫|凯露|at,qq=123456'
#                 ↑                        ↑
#自主修改       bot的昵称，用“|”隔开   bot的QQ号，用于检测@bot
#再修改橙色字体和jewel_limit（宝石数）和tenjo_limit（天井券）
#本抽签版抽卡还有一个util.py
#本抽卡 贡献者Lost℃  前川未来
import os
import random
...
BOT_NICKNAME = r'镜华|xcw|XCW|at,qq=1764461263'
...
async def set_pool(bot, ev: CQEvent):
    ...
    if not name:
        ...
        name = '泳裝-流夏'
    elif name in ('泳装初音', '水初音', '水星法', '泳装星法'):
        name = '泳裝-初音'
    elif name in ('泳装纯', '水纯', '水黑骑', '泳装黑骑'):
        name = '泳裝-纯'
    elif name in ('万圣限定'):
        await bot.finish(ev,'请选择以下卡池\n> 选择卡池 万圣忍\n> 选择卡池 万圣美咲\n> 选择卡池 万圣小仓唯')
    elif name in ('万圣忍', '瓜忍'):
...
async def check_limit_num(bot, ev: CQEvent):
    if lmtw.check(ev.user_id):
        await bot.send(ev, f'花琳正在清点宝石', at_sender=True)
...
async def gacha_d(bot, ev: CQEvent):
    ...
    if m:
        ...
        if random.random() <= 0.2:
            ...
            await bot.send(ev, f'宝石被珠希偷走了 >_< 就↗剩↘150宝石了')
...
async def gacha_1(bot, ev: CQEvent):
    ...
    if m:
        ...
        await bot.send(ev, f'素敵な仲間が増えますよ！\n{res}\n{SWITCH_POOL_TIP}\n※发送“镜华宝石”可以查看还剩多少宝石', at_sender=True)
...
async def kakin(bot, ev: CQEvent):
    if ev.user_id not in bot.config.OWNER:
        return
    count = 0
    for m in ev.message:
