...
hw = Blueprint('hw',__name__,template_folder='templates',static_folder=static_folder)
bot = nonebot.get_bot()
app = bot.server_app
...
@sv.on_rex('^查找.{0,}图$')
async def homework_query(bot, ev):
    uid = ev.user_id
    #文件查找
    ...
    arv = arv[2:l-2]
    msg = ''
    for path in homework_folder.iterdir():
        if path.name.find(arv) >= 0:
            msg += f'[CQ:image,file=file:///{path.as_posix()}]'
    if msg == '':
        await bot.send(ev,f'没有找到{arv}图！', at_sender=True)
        return        
...
@hw.route('/uploader', methods=['GET', 'POST'])
async def upload_file():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            file.save(homework_folder.joinpath(filename).as_posix())
            return 'success'
        return 'failed'
...
@hw.route("/del", methods=['DELETE'])
async def del_file():
    filename = request.args.get('filename')
    pwd = request.args.get('password')
    if pwd != HwManagePwd:
        return 'wrong password'
    file_dir = homework_folder.joinpath(filename)
    if file_dir.exists():
        file_dir.unlink()
        return 'success'
    return 'error'
...
@hw.route('/get/homeworklist')
async def get_homework_list():
    homework_list = []
    for path in homework_folder.iterdir():
        if allowed_file(path.name):
            homework_list.append({'filename': path.name, 'size': path.stat().st_size})
    return jsonify(homework_list)


@sv.on_fullmatch("相册",only_to_me=True)
async def get_uploader_url(bot, ev):
    cfg = config.__bot__
    await bot.send(ev,f'http://{public_address}:{cfg.PORT}/homework')
