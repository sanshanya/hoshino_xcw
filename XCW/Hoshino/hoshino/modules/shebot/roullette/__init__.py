...
@sv.on_fullmatch(["帮助轮盘决斗"])
async def bangzhu(bot, ev):
    await bot.send(ev, sv_help, at_sender=True)
    
@sv.on_fullmatch(( '轮盘决斗'))
async def roulette(bot: HoshinoBot, ev: Event):
    try:
        session = ActSession.from_event('轮盘决斗', ev, max_user=3, usernum_limit=True)
        interact.add_session(session)
        await bot.send(ev, '游戏开始,目前有1位玩家，还缺1名玩家，发送"参与决斗"加入游戏')
    except ValueError as e:
        await bot.finish(ev, f'{e}')

@sv.on_fullmatch('参与决斗')
async def join_roulette(bot: HoshinoBot, ev: Event):
    session = interact.find_session(ev, name='轮盘决斗')
    if not session: #session未创建
        await bot.send(ev, '游戏未创建，发送轮盘决斗创建游戏')
        return #不处理
    try:
...
@interact.add_action('轮盘决斗', (f'{MessageSegment.face(169)}', '开枪'))
